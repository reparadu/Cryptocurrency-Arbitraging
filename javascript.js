...
function main () {
  ...
  function retrieveData_CO_ETH(){
    $.getJSON("https://api.coinone.co.kr/orderbook/?currency=eth", function(data) {
      ...
      var buyETH_CO = $("#CO-ETH_buychoice").text(data["ask"][pointCO]["price"]);
      var sellETH_CO = $("#CO-ETH_sellchoice").text("Sell : " + data["bid"][pointCO]["price"]);

      
      $("#fundAmountCO").text("Initial fund : " + fundAmountCO + " SKW");
      $("#buyETH_CO").text(buyETH_CO);        
      $("#Amount_ETH").text("Amount ETH : " + (fundAmountCO / buyETH_CO));
